    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbar">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <h6 class="navbarText font-weight-bolder mb-0">Dashboard / Students Managements</h6>
        </nav>
        ...
        <div class="row">
          <div class="col-12">
            <div class="card my-4">
                  ...
                  <div class="col-md">
                    <div class="shadow-primary border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center" id="cardTop">
                      <h6 class="text-capitalize ps-3" id="titreTop">
                        <img src="{{ asset('student.png') }}" alt="Your Image" id="imageCardTop" class="img-fluid me-2">
                        Students Table
                      </h6>
                      <form action="{{route('students.create')}}" method="GET" class="m-1">
              ...
              <form id="filterForm" action="{{ route('students.filter') }}" method="GET">
                <div class="row mb-4">
                    <div class="col-md-8">
                        <select name="class_filter" class="form-control" style="border: 1px solid;padding:10px">
                            <option value="" selected disabled>Select Class</option>
                            @foreach($classes as $classe)
                                <option value="{{ $classe->id }}">{{ $classe->grade_level}} {{ $classe->class_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                      <button type="submit" class="btn btn-dark" id="applyFilters">Apply Filters</button>
                      <a href="{{route('students.index')}}"><button type="button" class="btn btn-outline-dark" id="clearFilters" onclick="clearFilters()">Clear Filters</button></a>
                    </div>
              ...
              <div class="table-wrapper">
                @include('students.students_table', ['students' => $students])
              </div>
